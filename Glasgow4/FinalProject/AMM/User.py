...
# 状态：balance_x, balance_y

# 函数：execute_swap(pool, amount_in, x_to_y, slippage_tolerance)

# 先用池子算预期输出 & slippage

# 如果实际输出低于用户通过 slippage 算出来的“最小可接受输出”，就回滚池子状态（交易失败）

# 2. 文献对应

# 这部分主要是**“交易者视角 + 成本 + slippage 策略”**：

# 成本拆解 & slippage 指标：

# 2_MEV_General/Don’t Let MEV Slip.pdf

# 这篇把 swap 成本拆成正常价格冲击和由排序/攻击产生的额外 slippage；

# 你在实验里要比较“有/无攻击时受害者支付的额外成本”，就可以说：指标设计参考了它。

# 受害者行为 & 防御：

# 3_Sandwich_V2/sandwich_attacks_ETH.pdf

# 里面统计了真实用户的 slippage 设置、被三明治后的损失，以及“拆单”等防御方式；

# 你可以说：为了分析 slippage tolerance 对损失的影响，我们参考了这篇的实证观察。

# slippage 策略博弈：

# 根目录 slippage_game_theory.pdf（你最好已经把 2202 那个重命名成这样）

# 它专门讨论“slippage 太小 → fail 太多；太大 → 被攻击薅干”的权衡；

# 你在实验里扫 slippage_tolerance 从 0.1% 到 20% 时，可以引用这篇解释趋势。

# → 总结一句话版本：

# User.py = “综合 Don’t Let MEV Slip.pdf、sandwich_attacks_ETH.pdf 和 slippage_game_theory.pdf，实现一个带 slippage 容忍度的受害者交易者模型”。